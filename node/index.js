...
class Vector2d {
     ...
     */
    static ccw(p1, p2, p3, delta = DELTA) {
    ...
    * defaults to 1e-10.
    */
    static segSegIntersection(ab, cd, delta = DELTA) {
        let [a, b] = ab;
        let [c, d] = cd;
    ...
    */
    static doesSegSegIntersect(a, b) {
    ...
    */
    static squaredDistanceBetween(p1, p2) {
        let x = p2[0] - p1[0];
        let y = p2[1] - p1[1];
        return x * x + y * y;
    }
    ...
    * @param p2 - Another point.
    */
    static manhattanDistanceBetween(p1, p2) {
    ...
    * @param l - A line
    */
    static distanceBetweenPointAndLine(p, l) {
        let [x0, y0] = p;
        let [[x1, y1], [x2, y2]] = l;
        let y = y2 - y1;
        let x = x2 - x1;
    ...
    * @param l - A line
    */
    static squaredDistanceBetweenPointAndLineSegment(p, l) {
    ...
    */
    static circumCenter(triangle) {
        ...
        let Sx = 0.5 * Vector2d.det3([sqLen(p1), p1[1], 1], [sqLen(p2), p2[1], 1], [sqLen(p3), p3[1], 1]);
        let Sy = 0.5 * Vector2d.det3([p1[0], sqLen(p1), 1], [p2[0], sqLen(p2), 1], [p3[0], sqLen(p3), 1]);
        let a = Vector2d.det3([p1[0], p1[1], 1], [p2[0], p2[1], 1], [p3[0], p3[1], 1]);
        let b = Vector2d.det3([p1[0], p1[1], sqLen(p1)], [p2[0], p2[1], sqLen(p2)], [p3[0], p3[1], sqLen(p3)]);
        return [Sx / a, Sy / a];
    }
    /**
    * <p>
    ...
    * @param cosθ
    */
    static reverseRotate(sinθ, cosθ, p) {
        return [
            +p[0] * cosθ + p[1] * sinθ,
            -p[0] * sinθ + p[1] * cosθ
        ];
    ...
    **/
    static translateThenRotatePs(v, sinθ, cosθ, ps) {
        const translate = Vector2d.translate(v);
        return ps.map(p => Vector2d.rotate(sinθ, cosθ, translate(p)));
    }
    /**
    ...
    * @param ps - The input points
    **/
    static rotateThenTranslatePs(sinθ, cosθ, v, ps) {
        return ps.map(p => Vector2d.translate(v, Vector2d.rotate(sinθ, cosθ, p)));
    }
}
/**
